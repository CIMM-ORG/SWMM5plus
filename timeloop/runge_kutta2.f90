...
!%
    subroutine rk2_toplevel_ETM_5
        ...
        !% --- ensure consistency between diagnostic and JB
        ...
        do istep = 1,2
            ...
            call update_auxiliary_variables_CC (                  &
                ...
                ! call util_utest_CLprint ('------- HHH  after update_aux...CC')

            !% --- zero an small depth adjustment for elements
            call adjust_element_toplevel (CC)

                ! call util_utest_CLprint ('------- III  after adjust_element...CC')
            ...
            if (N_nJM > 0) then 
                ...
                if (Npack > 0) then 
                    ...
                    ! call util_utest_CLprint ('------- KKK  after update weights JB for JB/Diag')
            end if
            ...
            call adjust_face_toplevel (fp_all_interior)    !% brhADDED 20230418
            
                ! call util_utest_CLprint ('------- LLL01  after adjust_face_toplevel')
            ...
            if (istep == 1) then 
                ...
                call rk2_store_conservative_fluxes (CCDiag) 
                    ...
                    ! call util_utest_CLprint ('------- MMM  after  step 1 store consertave fluxes CCDiag')
            end if
            ...
            if (N_nJM > 0) then 
                ...
                call adjust_element_toplevel (JM)
                    ...
                    ! call util_utest_CLprint ('------- QQQ  after adjuste element JB JM')
            ...
            !%     Data without transferring ghost elements
            !%================================
            ...
            !% contains the Diag-Adjacent element.
            !%================================
            ...
            if (N_diag > 0) then
                ...
                !%     non-adjacent face has flowrate changed for consistency.
                call face_velocities(fp_Diag_all, .true.)

                    ! call util_utest_CLprint ('------- VVV  after face velocities')
            end if
...
!%
    subroutine rk2_toplevel_ETM_4 ()
        ...
        !call junction_face_terms (fp_JB_interior, istep)    
        ...
        call adjust_face_toplevel (fp_all_interior)    
        ...
        call diagnostic_toplevel (ep_Diag, fp_Diag_interior, istep)  
...
!%
    subroutine rk2_toplevel_ETM_3 ()
        ...
        ! call pack_zero_depth_interior_faces (fp_notJB_interior)
        ...
        ! call face_interpolation(fp_JB_interior,whichTM,.true.,.true.,.true.)  
        ...
        ! call adjust_zero_or_small_depth_identify_NEW(JB,.false.)
        ! call adjust_zero_or_small_depth_identify_NEW(JM,.false.)
        ! call pack_zero_depth_interior_faces (fp_JB_interior)

        !      !  ! ! ! ! ! ! ! ! ! ! ! ! ! call util_utest_CLprint ('------- VVV01 after adjust...JM')

        ! !% --- interpolation to get the non-flow values on faces of junction
        ! !%     skipQ = true
        ! call face_interpolation(fp_JB_interior,whichTM,.true.,.false.,.true.)
...
!%    
    subroutine rk2_toplevel_ETM_2 ()
        ...
        ! sync all
        ! call face_interpolation(fp_all_interior,whichTM,.false.,.false.,.false.)
        ! call util_crashstop(72129873)
        ...
        ! !%     skipQ = true
        !     ! print *, ' '
        !     ! print *, 'fp_JB_interior', npack_faceP(fp_JB_interior)
        !     ! print *, faceP(1:npack_faceP(fp_JB_interior),fp_JB_interior)
        !     ! print *, ' '
...
!%
    subroutine rk2_toplevel_ETMAC ()
        ...
        ! sync all
        ! call face_interpolation(fp_all_interior,ALLtm,.false.,.false.,.false.)
...
!%
    subroutine rk2_store_conservative_fluxes (faceset)
        ...
        case (JBDiag)
            npack => npack_faceP(fp_JBDiag_all)
            if (npack > 0) then 
                thisF => faceP(1:npack,fp_JBDiag_all)
                faceR(thisF,fr_Flowrate_Conservative) = faceR(thisF,fr_Flowrate)
            end if
        case (JB)
            npack => npack_faceP(fp_JB_interior)
            if (npack > 0) then
                thisF => faceP(1:npack,fp_JB_interior)
                faceR(thisF,fr_Flowrate_Conservative) = faceR(thisF,fr_Flowrate)
            end if
        case default
            print *, 'CODE ERROR: unexpected case default'
