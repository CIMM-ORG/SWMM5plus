module egg_shaped_conduit
    ...
    !%-----------------------------------------------------------------------------
    !% Description:
    !% egg_shaped conduit geometry
    !%

    private

    public :: egg_shaped_depth_from_volume
    public :: egg_shaped_area_from_depth_singular
    public :: egg_shaped_topwidth_from_depth
    public :: egg_shaped_topwidth_from_depth_singular
    public :: egg_shaped_perimeter_from_depth
...
!%==========================================================================
!%
    subroutine egg_shaped_depth_from_volume (elemPGx, Npack, thisCol)
        !%-----------------------------------------------------------------------------
        !% Description:
        !% Only applies on conduits (or non-surcharged egg_shaped conduits)
        ...
        call xsect_table_lookup &
            (YoverYfull, AoverAfull, YEgg, thisP)  
...
!%
    subroutine egg_shaped_topwidth_from_depth (elemPGx, Npack, thisCol)
        ...
        !%-----------------------------------------------------------------------------
        !% Description:
        !% Computes the topwidth from a known depth in a egg_shaped conduit
        !%-----------------------------------------------------------------------------
        integer, target, intent(in) :: elemPGx(:,:), Npack, thisCol
        integer, pointer :: thisP(:)
        ...
        call xsect_table_lookup &
            (topwidth, YoverYfull, TEgg, thisP) 
...
!%
    subroutine egg_shaped_perimeter_from_depth (elemPGx, Npack, thisCol)
        ...
        !%-----------------------------------------------------------------------------
        !% Description:
        !% Computes the perimeter from a known depth in a egg_shaped conduit
        !%-----------------------------------------------------------------------------
        integer, target, intent(in) :: elemPGx(:,:), Npack, thisCol
        integer, pointer :: thisP(:)
        ...
        call xsect_table_lookup &
            (hydRadius, YoverYfull, REgg, thisP)  

        hydRadius(thisP) = 0.1931 * fulldepth(thisP) * hydRadius(thisP)
    ...
    end subroutine egg_shaped_perimeter_from_depth
!%
!%==========================================================================
!%==========================================================================
!%
...
!%
    subroutine egg_shaped_hyddepth_from_topwidth (elemPGx, Npack, thisCol)
        ...
        !%-----------------------------------------------------------------------------
        !% Description:
        !% Computes the hydraulic (average) depth from a known depth in a egg_shaped conduit
        !%-----------------------------------------------------------------------------
...
!%
    real(8) function egg_shaped_hydradius_from_depth_singular (indx,depth) result (outvalue)
        ...
        !%-----------------------------------------------------------------------------
        !% Description:
        !% Computes hydraulic radius from known depth for a egg_shaped cross section of
        !% a single element
        !%-----------------------------------------------------------------------------
        integer, intent(in) :: indx
        real(8), intent(in) :: depth
...
!%==========================================================================
!%
    real(8) function egg_shaped_normaldepth_from_sectionfactor_singular &
