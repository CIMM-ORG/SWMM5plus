...
if has_output_path:
    ...
    plot_dir   = output_dir+'/'+'plots_'+time_now
else:
    ...
    if(x[0:5]=='link_'):
        ...
        for parallel_file in h5_file_lists[1:]:
            ...
            if (print_plots):
                ...
                plt.plot(time,link_Q_serial,'--',color='xkcd:red',label='1')
                ii = 1
                for parallel_file in h5_file_lists[1:]:
                    ii = ii+1
                    z_parallel = get_array_from_dset(parallel_file,x)
                    link_Q_parallel = z_parallel[:,3] 
                    link_Y_parallel = z_parallel[:,2] 
                    plt.plot(time,link_Q_parallel,'--',label=str(ii))
                plt.xlabel('Time (hrs.)')
    ...
    if((x[0:10]=='node_face_') or (x[0:10]=='node_elem_')):
        ...
        for parallel_file in h5_file_lists[1:]:
            ...
            if (print_plots):
                ...
                plt.figure(2)
                plt.plot(figsize=(8,6))
                plt.plot(time,node_H_serial,'--',color='xkcd:red',label='1')
                ii = 1
                for parallel_file in h5_file_lists[1:]:
                    ii = ii+1
                    z_parallel = get_array_from_dset(parallel_file,x)
                    if is_nJ2:  
                        ...
                        node_H_parallel = (z_parallel[:,5]) * Yf # take the JM peizometric head
                    plt.plot(time,node_H_parallel,'--',label=str(ii))
