module define_xsect_tables
    ...
    !%
    !% Description:
    !% Geometry cross-section tables (non-dimensionalized)
    !%
    !% Methods
    !% These are tables extracted directly from EPA-SWMM-C
    !%==========================================================================
...
! Arch Shape
!==========================================================================
    !% get Y/Yfull from uniform  A/Afull (generated from the table below)
    integer, parameter :: NYArch = 26
    real(8), target, dimension(NYArch) :: YArch = (/0.0,    0.06,   0.1,    0.14,   0.176,  0.208, &
        0.24,   0.272,  0.304,  0.336,  0.368,  0.4,    0.432,  0.464,  0.496,  0.528, &
        0.56,   0.592,  0.6267, 0.6622, 0.7,    0.74,   0.7829, 0.8286, 0.88,   1.0/)
    !% get A/Afull from uniform  Y/Yfull 
    integer, parameter :: NAArch = 26
    real(8), target, dimension(NAArch) :: AArch = (/0.0000, 0.0200, 0.0600, 0.1000, 0.1400, 0.1900, &
    ...
    !% get S/Sfull from uniform  A/Afull (Used for hydraulic radius)
    integer, parameter :: NRArch = 26
    real(8), target, dimension(NRArch) :: RArch = (/0.0100, 0.0983, 0.1965, 0.2948, 0.3940, 0.4962, &
    ...
    !% get T/Tmax from uniform  Y/Yfull
    integer, parameter :: NTArch = 26
    real(8), target, dimension(NTArch) :: TArch = (/0.0, 0.6272, 0.8521, 0.9243, 0.9645, 0.9846, &
...
! Basket Handle Shape
!==========================================================================
    !% get Y/Yfull from uniform   A/Afull
    integer, parameter :: NYBasketHandle = 51
    real(8), target, dimension(NYBasketHandle) :: YBasketHandle = (/0.0, 0.04112, 0.0738,  0.1,     0.12236, &
    ...
    real(8), target, dimension(NABasketHandle) :: ABasketHandle = (/0.0000, 0.0173, 0.0457, 0.0828, &
        ...
        0.9880, 1.000/)
    !% get R/Rfull from uniform   Y/Yfull
    integer, parameter :: NRBasketHandle = 26
    real(8), target, dimension(NRBasketHandle) :: RBasketHandle = (/0.0100, 0.0952, 0.1890, 0.2730, &
    ...
    real(8), target, dimension(NTBasketHandle) :: TBasketHandle = (/0.0, 0.49, 0.667, 0.82,  0.93,  &
...
!==========================================================================
    ...
    real(8), target, dimension(NYCatenary) :: YCatenary = (/0.0, 0.02974, 0.06439, 0.08433, 0.10549, &
        ...
        1.0 /)
    !% get A/Afull from uniform  Y/Yfull (generated from the table above)
    integer, parameter :: NACatenary = 51
    real(8), target, dimension(NACatenary) :: ACatenary = (/0.0, 0.01345, 0.02592, 0.03747, 0.05566, &
    ...
    real(8), target, dimension(NSCatenary) :: SCatenary = (/0.0, 0.00605, 0.01455, 0.0254,  0.03863, &
    ...
    real(8), target, dimension(NTCatenary) :: TCatenary = (/0.0,    0.6667,  0.8222,  0.9111,  &
...
!==========================================================================
    ...
    real(8), target, dimension(NYCirc) :: YCirc = (/0.0, 0.05236, 0.08369, 0.11025, 0.13423, 0.15643,    &
    ...
    real(8), target, dimension(NACirc) :: ACirc = (/0.0, 0.00471, 0.0134, 0.024446, 0.0374, 0.05208,     &
    ...
    real(8), target, dimension(NRCirc) :: RCirc = (/0.01, 0.0528, 0.1048, 0.1556, 0.2052, 0.2540,        &
    ...
    !% get T/Tmax from  Y/Yfull
    integer, parameter :: NTCirc = 51
    real(8), target, dimension(NTCirc) :: TCirc = (/0.0, 0.2800, 0.3919, 0.4750, 0.5426, 0.6000, 0.6499, &
...
!==========================================================================
    ...
    real(8), target, dimension(NYEGG) :: YEgg = (/0.0, 0.04912, 0.08101, 0.11128, 0.14161, 0.16622, &
    ...
    real(8), target, dimension(NAEgg) :: AEgg = (/0.0000, 0.0150, 0.0400, 0.0550, 0.0850, 0.1200, &
    ...
    real(8), target, dimension(NREgg) :: REgg = (/0.0100, 0.0970, 0.2160, 0.3020, 0.3860, 0.4650, &
    ...
    real(8), target, dimension(NTEgg) :: TEgg = (/0.0, 0.2980, 0.4330, 0.5080, 0.5820, 0.6420, 0.6960, &
...
!==========================================================================
    ...
    real(8), target, dimension(NYGothic) :: YGothic = (/0.0, 0.04522, 0.07825, 0.10646, 0.12645, &
    ...
    real(8), target, dimension(NAGothic) :: AGothic = (/0.0, 0.00885, 0.01769, 0.02895, 0.04124, &
    ...
    real(8), target, dimension(NSGothic) :: SGothic = (/0.0, 0.005,   0.0174,  0.03098, 0.04272, &
    ...
    real(8), target, dimension(NTGothic) :: TGothic = (/0.0,   0.286,   0.643,   0.762,   0.833,  &
...
!==========================================================================
    !% get Y/Yfull from uniform  A/Afull (generated from the table below)
    integer, parameter :: NYHorizEllip = 26
    real(8), target, dimension(NYHorizEllip) :: YHorizEllip = (/0.0000, 0.08,   0.14,   0.1886, 0.2343, 0.275,  &
        0.3111, 0.344,  0.3745, 0.4033, 0.43,   0.4554, 0.48,   0.52,   0.5446, 0.57,   0.5967, 0.6255, &
        0.656,  0.6889, 0.725,  0.7657, 0.8114, 0.86,   0.92,   1.0/)
    !% get A/Afull from uniform  Y/Yfull 
    integer, parameter :: NAHorizEllip = 26
    real(8), target, dimension(NAHorizEllip) :: AHorizEllip = (/0.0000, 0.0150, 0.0400, 0.0650, 0.0950, 0.1300, &
    ...
    !% get S/Sfull from uniform  A/Afull (Used for hydraulic radius)
    integer, parameter :: NRHorizEllip = 26
    real(8), target, dimension(NRHorizEllip) :: RHorizEllip = (/0.0100, 0.0764, 0.1726, 0.2389, 0.3274, 0.4191, &
    ...
    !% get T/Tmax from uniform  Y/Yfull
    integer, parameter :: NTHorizEllip = 26
    real(8), target, dimension(NTHorizEllip) :: THorizEllip = (/0.0000, 0.3919, 0.5426, 0.6499, 0.7332, 0.8000, &
...
! Horseshoe Shape
!==========================================================================
    !% Y/Yfull from uniform  A/Afull
    integer, parameter :: NYHorseShoe = 51
    real(8), target, dimension(NYHorseShoe) :: YHorseShoe = (/0.0, 0.04146, 0.07033, 0.09098, 0.10962, &
    ...
    real(8), target, dimension(NAHorseShoe) :: AHorseShoe = (/0.0000, 0.0181, 0.0508, 0.0908, 0.1326, &
    ...
    real(8), target, dimension(NRHorseShoe) :: RHorseShoe = (/0.0100, 0.1040, 0.2065, 0.3243, 0.4322, &
    ...
    real(8), target, dimension(NTHorseShoe) :: THorseShoe = (/0.0000, 0.5878, 0.8772, 0.8900, 0.9028, &
...
!==========================================================================
    ...
    real(8), target, dimension(NYSemiCircular) :: YSemiCircular = (/0.0, 0.04102, 0.07407, 0.1,     &
    ...
    real(8), target, dimension(NASemiCircular) :: ASemiCircular = (/0.0, 0.00975, 0.01950, 0.03149, &
    ...
    real(8), target, dimension(NSSemiCircular) :: SSemiCircular = (/0.0, 0.00757, 0.01815, 0.03,    &
    ...
    real(8), target, dimension(NTSemiCircular) :: TSemiCircular = (/0.0, 0.5488,  0.8537,  1.0000,  &
...
!==========================================================================
    ...
    real(8), target, dimension(NYSemiEllip) :: YSemiEllip = (/0.0, 0.03075, 0.05137, 0.07032, 0.09,   &
    ...
    real(8), target, dimension(NASemiEllip) :: ASemiEllip = (/0.0, 0.01301, 0.02897, 0.04911, 0.06984, &
    ...
    real(8), target, dimension(NSSemiEllip) :: SSemiEllip = (/0.0, 0.00438, 0.01227, 0.02312, 0.03638, &
    ...
    real(8), target, dimension(NTSemiEllip) :: TSemiEllip = (/0.0, 0.7000,  0.9800,  1.0000,  1.0000,  &
...
!==========================================================================
    ...
    real(8), target, dimension(NYVertEllip) :: YVertEllip = (/0.0,    0.08,   0.1333, 0.18,   0.2178, 0.2533, 0.288, &
    ...
    real(8), target, dimension(NAVertEllip) :: AVertEllip = (/0.0000, 0.0100, 0.0400, 0.0700, 0.1000, 0.1400, 0.1850, &
    ...
    real(8), target, dimension(NRVertEllip) :: RVertEllip = (/0.0100, 0.1250, 0.2436, 0.3536, 0.4474, 0.5484, 0.6366, &
    ...
    real(8), target, dimension(NTVertEllip) :: TVertEllip = (/0.0000, 0.3919, 0.5426, 0.6499, 0.7332, 0.8000, 0.8542, &
