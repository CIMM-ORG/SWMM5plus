...
!% and downstream adjacent element or BC
 ...
 do ii=1,N_link
    ...
    eStart = minloc(elem2I(:,e2i_link_Pos),1,(  (elem2I(:,e2i_link_ID) == Lidx) .and. (elem2I(:,e2i_link_Pos) /= nullvalueI) ) )
    eEnd   = maxloc(elem2I(:,e2i_link_Pos),1,(  (elem2I(:,e2i_link_ID) == Lidx) .and. (elem2I(:,e2i_link_Pos) /= nullvalueI) ) )
    nelem = eEnd - eStart + 1
