...
!        print *,trim(subroutine_name), ii
!        print *,elem2R(1:6,e2r_Volume),' = start loop'        
!        print *,faceR(3,fr_flowrate), elem2R(4,e2r_flowrate), faceR(4,fr_flowrate), elem2R(5,e2r_flowrate), faceR(5,fr_flowrate)
!        !print *, elem2R(1:5,e2r_Velocity)     
!        print *
!        
        call sve_rk2_step &
...
!        !print *,trim(subroutine_name), ii
!        print *,elem2R(1:5,e2r_Volume_new),' = after sve_rk2_step'
...
!         
        ...
        call element_geometry_update &
            (elem2R, elem2I, elem2YN, e2r_Volume_new, &
             ...
             faceI, bcdataDn, bcdataUp, steptime)

!        print *, elem2R(1:5,e2r_Volume_new),' = after geometry update'
!        print *, elem2R(1:5,e2r_Velocity_new)
!        !print *, elem2YN(1:5,e2YN_isSmallVolume)
!        !print *, elem2R(1:5,e2r_SmallVolume)        
!        !print *, elem2R(1:5,e2r_SmallVolumeRatio)   
!        print *
!        
...
!        print *, elem2R(1:5,e2r_Volume_new),' = after dynamics update'
!        print *, elem2R(1:5,e2r_Velocity_new)
!        print *
!
        call face_update &
...
!        print *, elem2R(1:5,e2r_Volume_new),' = after adjust V'
!        print *, elem2R(1:5,e2r_Velocity_new)
!        print *
!
        if (ii==1) then
            !% store the net face fluxes that are used for volume advance.
            call diagnostic_element_volume_conservation_fluxes (elem2R, elem2I, faceR)
        endif
    end do
    ...
    call overwrite_old_values &
        (elem2R, elem2I, e2r_Velocity, e2r_Velocity_new, &
         e2r_Volume, e2r_Volume_new, e2i_elem_type, eChannel)   
         
    call overwrite_old_values &
        (elemMR, elemMI, eMr_Velocity, eMr_Velocity_new, &
         eMr_Volume, eMr_Volume_new, eMi_elem_type, eJunctionChannel)   
...
!%  HACK -- needs factor for angle with main channel
 do mm=1,upstream_face_per_elemM
    ...
    where ( (elemMI(:,eMi_elem_type) == eJunctionChannel).and. &
            (elemMI(:,upstream_face_per_elemM) >= mm) )
        kcM = kcM + dt * fQ(iup)
...
!%  Junctions (downstream faces)
 do mm=1,dnstream_face_per_elemM
    ...
    where ( (elemMI(:,eMi_elem_type) == eJunctionChannel) .and. &
            (elemMI(:,dnstream_face_per_elemM) >= mm) )
        kcM = kcM - dt * fQ(idn)
