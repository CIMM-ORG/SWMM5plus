...
contains
   ...
   subroutine bc_applied_onface &
      ...
      call bc_onface(faceR, faceI, elem2R, bcdataDn, thisTime)
      call bc_onface(faceR, faceI, elem2R, bcdataUp, thisTime)
      ...
      call  bc_ghost_othervalues &
         (elem2R, elem2I, faceR, faceI, bcdataDn, e2i_Mface_d, e2r_Velocity_new, &
         fr_Area_u, fr_Velocity_u, fr_HydDepth_u, fr_Eta_u)
   ...
   subroutine bc_onface &
      (faceR, faceI, elem2R, bcdata, thisTime)
      !
      ...
      do ii=1,size(bcdata)
         ...
         faceI(thisloc,fi_BC_ID) = bcdata(ii)%Idx
   ...
   subroutine bc_face_othervalues &
      ...
      e2rset = (/e2r_Topwidth, e2r_Area,  e2r_HydDepth /)
      frset  = (/fr_Topwidth,  fr_Area_d, fr_HydDepth_d /)
      ...
      do ii=1,size(bcdata)
         ...
         froudeMax => setting%Limiter%BC%FroudeInflowMaximum
         ...
         faceR(fID,fr_HydDepth_u) =  faceR(fID,fr_HydDepth_d)
         ...
         if (bcdata(ii)%Category /= bc_category_elevation) then
            ...
            ! HACK need to review and document this subroutine
            ...
            if (faceR(fID,fr_Area_d) > zeroR) then
               ...
               faceR(fID,fr_HydDepth_d) = setting%Zerovalue%Depth
            ...
            if ((thisFroudeNumber > froudeMax) .and. &
               (setting%Limiter%BC%UseInflowFroudeNumberLimiter)) then
               !% if max froude is exceeded, we need to adjust the inflow area and
               ...
               ! Depth       => elem2R(eID,e2r_HydDepth)
               Depth       => faceR(fID,fr_HydDepth_d)
               ...
               faceR(fID,fr_HydDepth_u) = thisDepth
               faceR(fID,fr_HydDepth_d) = thisDepth
   ...
   !==========================================================================
   !
   subroutine bc_ghost_othervalues &
      (elem2R, elem2I, faceR, faceI, bcdata, e2i_Mface_dir, e2r_Velocity_new,   &
      fr_Area_dir, fr_Velocity_dir, fr_HydDepth_dir, fr_Eta_dir)
      ...
      integer,       target, intent(in)      :: faceI(:,:)
      type(bcType),  target, intent(in)      :: bcdata(:)
      integer,               intent(in)      :: e2i_Mface_dir, e2r_Velocity_new
      integer,               intent(in)      :: fr_Velocity_dir, fr_HydDepth_dir
      integer,               intent(in)      :: fr_Eta_dir, fr_Area_dir

      integer,   pointer :: thisghost, thisloc, thiscat, thisface
      integer :: colE(5), colF(5), mm, ii
      ...
      do ii=1,size(bcdata)
         ...
         thisloc   => bcdata(ii)%ElemInsideID
         thiscat   => bcdata(ii)%category
         thisface  => faceI(elem2I(thisloc,e2i_Mface_dir),fi_idx)
         if (thiscat == bc_category_elevation) then
            ! store all face values except eta on ghost cell
            colE(1:5) = (/e2r_Area,    e2r_Flowrate,    e2r_Velocity_new,    e2r_Topwidth, e2r_HydDepth /)
            colF(1:5) = (/fr_Area_dir, fr_Flowrate,     fr_Velocity_dir, fr_Topwidth,  fr_HydDepth_dir /)
            elem2R(thisghost,colE(1:5)) = faceR(thisface,colF(1:5))
         elseif (thiscat == bc_category_inflowrate) then
            !%   store all face values except flow rate and velocity on ghost cell.
            colE(1:4) = (/e2r_Area,     e2r_Topwidth, e2r_HydDepth,    e2r_Eta /)
            colF(1:4) = (/fr_Area_dir,  fr_Topwidth,  fr_HydDepth_dir, fr_Eta_dir /)
            elem2R(thisghost,colE(1:4)) = faceR(thisface,colF(1:4))
         else
            print *, 'error: unexpected value for bcdata%category of ',thiscat,' in ',subroutine_name
            stop
         endif
      ...
      ! print *, faceR(:,fr_Velocity_dir)
      ! print *, thisghost, thisface
