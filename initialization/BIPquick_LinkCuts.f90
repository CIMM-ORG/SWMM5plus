...
!%***********************************************************************************
    ...
    public :: init_partitioning_BIPquick_LC

    real(8), parameter :: precision_matching_tolerance = 1.0D-6 ! a tolerance parameter for whether or not two real(8) numbers are equal
...
contains
    ...
    subroutine init_partitioning_BIPquick_LC()
    ! ----------------------------------------------------------------------------------------------------------------
    !
    ! Description:
    ...
    !
    ! -----------------------------------------------------------------------------------------------------------------
        character(64) :: subroutine_name = 'BIPquick_LC_subroutine'
        ...
        integer       :: effective_root = nullValueI
        integer       :: effective_root_last, effective_root_repeat
        ...
        do mp = 1, num_images()
            ...
            if ( mp == num_images() ) then
                do ii = 1, size(node%I, 1)
                    if ( node%I(ii, ni_node_type) == nBCdn ) then
                        print*, "Last image shortcut"
                        call trav_subnetwork(node%I(ii, ni_idx), image)
                        exit
                    endif
                enddo
                exit
            ...
            partition_threshold = nint(max_weight/real(num_images() - mp + 1, 8))

            print*, "SWEEP", image
            do ii = 1, size(node%I, 1)
                print*, B_nodeR(ii, :)
            enddo
            print*, max_weight, mp, partition_threshold

            if ( effective_root /= nullValueI ) then
                effective_root_last = effective_root
                effective_root_repeat = 0
            endif
            ...
            if ( effective_root == effective_root_last ) then
                effective_root_repeat = effective_root_repeat + 1
            else
                effective_root_repeat = 0
            endif
            ...
            print*, "Sweep", mp, "ideal_exists", ideal_exists, "effective_root", effective_root, &
            "spanning_link", spanning_link, "partition_threshold", partition_threshold

            if ( ( ideal_exists .eqv. .true. ) .and. (node%I(effective_root, ni_node_type) == nJ2 ) ) then
            ...
            do while ( (spanning_link == nullValueI) .and. ( ideal_exists .eqv. .false. ) )
                ...
                call trav_casethree(effective_root, effective_root_repeat, spanning_link, image, &
                    partition_threshold, max_weight, ideal_exists, phantom_node_idx, phantom_link_idx)

            end do

            !% Or Case 2
            if ( spanning_link /= nullValueI ) then
    ...
    function calc_effective_root(ideal_exists, max_weight, partition_threshold) result (effective_root)
        ...
        do ii=1, size(node%I,1)
            ...
            .le. precision_matching_tolerance )  then
    ...
    subroutine calc_spanning_link(spanning_link, partition_threshold)
        ...
        do jj=1, size(link%I,1)
            ...
            weight_range(twoI) = weight_range(oneI) + link%I(jj, li_N_element)
            ...
            if( (weight_range(oneI) .lt. partition_threshold) .and. &
            (partition_threshold .lt. weight_range(twoI)) .and. &
    ...
    function calc_phantom_node_loc(spanning_link, partition_threshold) result(length_from_start)
        ...
        link_weight = link%I(spanning_link, li_N_element)
        ...
        !% Ensures that the cuts are at least one element length away from nodes
        link_length_less_NEL = link%R(spanning_link, lr_Length)
        if ( length_from_start .lt. elem_nominal_length ) then
            length_from_start = elem_nominal_length
        elseif ( length_from_start .gt. link_length_less_NEL ) then
            length_from_start = link_length_less_NEL
        end if
    ...
    subroutine phantom_node_generator &
        (spanning_link, phantom_node_weight, phantom_node_start, phantom_node_idx, phantom_link_idx)
        !-----------------------------------------------------------------------------
        !
        ...
        real(8), intent(in)       :: phantom_node_start, phantom_node_weight
        integer, intent(in)       :: spanning_link
        integer :: upstream_node_list(3)
        ...
        !% The phantom node totalweight is calculated outside of the generator
        B_nodeR(phantom_node_idx, totalweight) = phantom_node_weight
        ...
        - link%I(spanning_link, li_N_element)
    ...
    !==========================================================================
    !
    subroutine trav_casethree(effective_root, effective_root_repeat, spanning_link, image, &
        partition_threshold, max_weight, ideal_exists, phantom_node_idx, phantom_link_idx)
    !-----------------------------------------------------------------------------
    !
    ...
    !-----------------------------------------------------------------------------
        ...
        integer, intent(in out)   :: effective_root, effective_root_repeat, spanning_link, image
        integer, intent(in out)   :: phantom_node_idx, phantom_link_idx
        real(8), intent(in out)   :: partition_threshold, max_weight
        logical, intent(in out)   :: ideal_exists
        integer   :: upstream_node_list(3), upstream_node
        real(8)   :: total_clipped_weight, link_length_less_NEL
        integer   :: jj
    ...
    !--------------------------------------------------------------------------
        ...
        upstream_node = upstream_node_list(oneI + effective_root_repeat)
        ...
        do jj=1,size(link%I,1)
            ...
            if (link%I(jj, li_Mnode_u) == upstream_node) then
                ...
                !% HACK - I'm leaving off here to go to dinner
                !% need to check that the totalweight for the effective root (3) is still 50.0

                !% The clipped weight is the upstream totalweight + the link weight - one element's length
                total_clipped_weight = B_nodeR(upstream_node, totalweight) &
                + link%I(jj, li_N_element) - oneI

                link_length_less_NEL = link%R(jj, lr_Length) - elem_nominal_length

                !% Call the phantom_node_generator on the upstream link one element length upstream
                !% of the effective node
                call phantom_node_generator &
                (jj, total_clipped_weight, link_length_less_NEL, phantom_node_idx, phantom_link_idx)
        ...
        B_nodeR(effective_root, directweight) - link%I(phantom_link_idx, li_N_element)
        ...
        print*, "Calling subnetwork carving on ", phantom_node_idx, partition_threshold
        ...
        !% Update the phantom node/link indices 
        phantom_node_idx = phantom_node_idx + 1
        phantom_link_idx = phantom_link_idx + 1
