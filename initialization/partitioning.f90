    ...
    subroutine partitioning_toplevel()
        ...
        !% Closing
            if (setting%Debug%File%partitioning) then
                ...
                !% --- Check to see if the default partitioning is working correctly for the hard-coded case
...
!%
    function init_partitioning_metric_partsizebalance() result(part_size_balance)
        !%------------------------------------------------------------------
        !% Description:
        !% This calculates the part size balance metric for the given partition 
        !% set (i.e. the output from any of the partitioning algorithms).  
        !% The part size balance metric is equal to the greatest number of
        !% elements assigned to a single processor minus the smallest number
        !% of elements assigned to a single processor.
        !%------------------------------------------------------------------
        !% Declarations:
        ...
        do ii = 1, size(node%I, 1)
            ...
            !% T--- The current image is the one to which the current link has been assigned
        ...
        !% --- The difference between the max and min number of elements per image 
        !%     is the part_size_balance objective metric
...
!%
    function init_partitioning_metric_connectivity() result(connectivity)
        ...
        !% The connectivity metric is equal to the sum of the ni_is_boundary
        !% column of the node%I array after the partition has been completed. 
        !%  Note: the ni_is_boundary column is given a value of 0 when the 
        !% node is an internal partition node, and is incremented by 1 for
        !% each additional partition the node touches.
        !%------------------------------------------------------------------
