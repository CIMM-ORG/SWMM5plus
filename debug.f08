...
!%  and an array of column indexes to use in an iterative call.
 ...
 if (setting%Debugout%elem2R) then
    ...
    ii=ii+1
    call debug_singlefile_open (debugfile(ii), 'elem2R','Timescale_u', e2r_Timescale_u)
    ii=ii+1
    call debug_singlefile_open (debugfile(ii), 'elem2R','Timescale_d', e2r_Timescale_d)
    ii=ii+1
 ...
 if (setting%Debugout%elemMR) then 
    print *, 'error: code for debug output of elemMR not completed in ',subroutine_name
    stop
 endif
...
!-------------------------------------------------------------------------- 
 ...
 debugfile%FileInfo%FilePath   = trim(setting%Debugout%FolderPath)//trim(setting%Debugout%FolderName)//'/'
 ...
 debugfile%FileInfo%WriteName  = trim(debugfile%FileInfo%FilePath) // &
